class Mapa {
    ...
    criarMarcadores = (data, dtInicial, dtFinal) => {
      ...
      if (baciasSelecionadas.length > 0) {
        data.forEach((estacao, index) => {
          if (baciasSelecionadas.includes(estacao.bacia.toUpperCase()) && estacao.chuva_acumulada >= Number(document.getElementById("mm-chuva").value)) {
        ...
        data.forEach((estacao, index) => {
          if (estacao.chuva_acumulada >= Number(document.getElementById("mm-chuva").value)) {
  ...
  window.addEventListener('load', async function () {
    ...
    const criarTabela = (objectList) => {
      document.getElementById('tabela').innerHTML = "";
      objectList.forEach((mediaBacia) => {
        let fontColor = mediaBacia.precipitacao_media > 0 ? '' : 'color:gray;'
        let tableRow = $(`<tr><th scope="row" style="${fontColor}">${mediaBacia.bacia}</th><td style="${fontColor}">${mediaBacia.precipitacao_media}</td><td style="${fontColor}">${mediaBacia.porcentagem_bacias_utilizadas}</td><td style="${fontColor}">${mediaBacia.bacias_totais}</td></tr>`);
    ...
    const atualizarTabela = () => {
      const atualizar = document.getElementById('forcarAtualizacao').checked;
      fetchData(`estacoes-meteorologicas/chuva-acumulada/media?dtInicial=${dtInicial.value}&dtFinal=${dtFinal.value}&atualizar=${atualizar}`).then((response) => {
        criarTabela(response);
      })
    }
  
    let datas = await fetchData('estacoes-meteorologicas/datas-coleta');
    let slider = new Slider(datas['datas'], document.getElementById("slider"), dtInicial, dtFinal, () => { atualizarMapa(); atualizarTabela() }, 24);
