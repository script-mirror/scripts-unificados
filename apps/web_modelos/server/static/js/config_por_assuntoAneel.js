var inputAssunto = document.getElementById("inputAssunto");
let botaoAdicionar = document.getElementById("btn-Adicionar");
let corpoTab = document.getElementById("corpoTabela");
let botaoSalvar = document.getElementById("btn-Salvar");
var objetoPalavrasSelecionadas = {};
var index = 0;
let btnSwitch_documentos = document.getElementById("nav-documentos")


$.ajax({
  method: "GET",
  url: "/middle/API/get/assuntos-Aneel",
  data: {},
}).done(function (objeto) {
  for (let idx in objeto) {
    row = objeto[idx];
    adicionaLinha(row["id"], row["assunto"]);
    index = row["id"];
  }
  objetoPalavrasSelecionadas = objeto;
});

botaoSalvar.addEventListener("click", function () {
  dataToSend = JSON.stringify(objetoPalavrasSelecionadas);

$.ajax({
    contentType: "application/json; charset=utf-8",
    dataType: "json",
    type: "POST",
    url: "/middle/API/set/assuntos-Aneel",
    data: dataToSend,
    success: function (data) {
      alert("Dados alterados com sucesso!");
    },
    error: function(){
      alert("Erro, tente novamente!");
    }
  });
});

botaoAdicionar.addEventListener("click", function () {
  if (inputAssunto.value == "") {
    alert("Complete os campos em branco");
  } else {
    index = parseInt(index) + 1;
    adicionaLinha(index, inputAssunto.value);
    objetoPalavrasSelecionadas.push({
      id: index,
      assunto: inputAssunto.value,
      
    });
    inputAssunto.value = "";
  }
});

function removerPalavra(elemento,assunto) {
  var arraypalavras = objetoPalavrasSelecionadas.filter(function(e){
    return e.assunto != assunto
  })
  objetoPalavrasSelecionadas = arraypalavras
  apagaLinha = elemento.parentNode.parentNode.remove();
}

function adicionaLinha(index, assunto) {
  let _html = "";
  _html += `<tr>
      <th class = 'hidden d-none data-index='${index}'>${index}</th>
        <td>${assunto} <button data-index='${index}' class="btn btn-outline-primary float-right botaoRemover" type="button"
          onclick="removerPalavra(this,'${assunto}')">X</button>
        </td>
      </tr>`;
  corpoTab.innerHTML += _html;
}



function relocate_processos()
{
     location.href = "/middle/config-processos-Aneel";
} 