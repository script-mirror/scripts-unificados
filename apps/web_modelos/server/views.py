...
def signup():
  send_whatsapp_message("debug", "ACESSO AO SIGNUP")
  form = RegistrationForm()
  if form.validate_on_submit():
    hashed_password = generate_password_hash(form.password.data, method='pbkdf2:sha256')
    new_user = User(username=form.username.data, email=form.email.data, password=hashed_password)
    send_whatsapp_message("debug", f"usuario: {form.username.data} - email: {form.email.data}")
...
# desativar futuramente
@bp.route('/carga_2')
@login_required
def carga_subsistema_2():
  return render_template('carga_subsistema.html')
