...
def download_deck_ds(**kwargs):
    try:
        ...
        dt_ref = (datetime.datetime.now().replace(hour=0, minute=0, second=0) + 
                 datetime.timedelta(days=1)) if not dt_ref else datetime.datetime.strptime(dt_ref, '%Y-%m-%d')
        
        print(f"Data de referência: {dt_ref}")
...
with DAG(
    ...
    inicio >> downloadCCEE_ds >> dbUpdater_ds >> fim